...
namespace KoreanAIO.Champions
{
    public class Syndra : ChampionBase
    {
        ...
        public Syndra()
        {
            ...
            {
                if (sender == Target && ModeManager.LastHit && sender.IsValidTarget(1300) && sender.IsEnemy && sender.Type == GameObjectType.AIHeroClient)
                {
            ...
            Obj_AI_Base.OnProcessSpellCast += delegate (Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
            {
                ...
                {
                if (!sender.IsDashing() && sender == Target && ModeManager.LastHit && sender.IsValidTarget(1300) && Q.IsReady  && sender.IsEnemy && sender.Type == GameObjectType.AIHeroClient)
                {    
                    if (Q.IsReady && sender.IsValidTarget(825))
                    {
                        Q.Cast(sender.ServerPosition);
