...
namespace KoreanAIO.Champions
{
    public class Syndra : ChampionBase
    {
        ...
        public Syndra()
        {
            ...
            {
                if (!sender.IsDashing && sender == Target && ModeManager.LastHit && sender.IsValidTarget(950) && Q.IsReady  && sender.IsEnemy && sender.Type == GameObjectType.AIHeroClient)
                {
            ...
            Obj_AI_Base.OnProcessSpellCast += delegate (Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
            {
                ...
                {
                if (sender == Target && ModeManager.LastHit && sender.IsValidTarget(950) && Q.IsReady  && sender.IsEnemy && sender.Type == GameObjectType.AIHeroClient)
                {
                    if (sender.IsValidTarget(825))
                    {
                        Q.Cast(sender.ServerPosition);
